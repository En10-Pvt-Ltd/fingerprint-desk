#!/usr/bin/env python3
# SPDX-License-Identifier: Apache-2.0
"""End-to-end check of vector line-shift fingerprinting on an
outline-text PDF.

    python app/tools/vector_check.py --pdf paper.pdf --pages 5 \
        --seeds 6101 6102 --out field-test-vector

Same shape as raster_check: emit v1/v2 + control, verify the formatting
guarantee on 300-dpi renders, then run the synthetic decode matrix and
the unmarked-control chance check. Synthetic-channel numbers verify
decoder logic only — physical go/no-go needs the M3 protocol.
"""
import argparse
import json
import os
import sys

HERE = os.path.dirname(os.path.abspath(__file__))
APP = os.path.dirname(HERE)
REPO = os.path.dirname(APP)
for p in (APP, REPO, os.path.join(REPO, "robust_decode")):
    sys.path.insert(0, p)

import fitz                   # noqa: E402
import numpy as np            # noqa: E402

from engine import vector_mark, channel_sim   # noqa: E402


def fail(msg):
    print(f"FAIL  {msg}")
    sys.exit(1)


def render_gray(pdf_bytes, pno, dpi=300):
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    pix = doc[pno].get_pixmap(dpi=dpi, colorspace=fitz.csGRAY)
    arr = np.frombuffer(pix.samples, np.uint8).reshape(pix.height,
                                                       pix.width).copy()
    doc.close()
    return arr


def render_png(pdf_bytes, pno, path):
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    doc[pno].get_pixmap(dpi=300).save(path)
    doc.close()
    return path


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--pdf", required=True)
    ap.add_argument("--pages", type=int, default=5)
    ap.add_argument("--seeds", type=int, nargs=2, default=[6101, 6102])
    ap.add_argument("--out", required=True)
    args = ap.parse_args()

    os.makedirs(args.out, exist_ok=True)
    wd = os.path.join(args.out, "work")
    os.makedirs(wd, exist_ok=True)
    src = open(args.pdf, "rb").read()
    pages = range(args.pages)

    print(f"[1] capacity analysis (first {args.pages} pages, 300-dpi frame)")
    ana = vector_mark.analyze(src, pages)
    total = sum(len(pg["triplets"]) for pg in ana)
    for pg in ana:
        note = "  [ROTATED CTM — unmarkable]" if pg["rotated"] else ""
        print(f"  page {pg['page_index']}: {len(pg['runs'])} line runs, "
              f"{len(pg['triplets'])} markable triplets{note}")
    print(f"  total slots: {total}")
    if total < 6:
        fail("fewer than 6 slots across the marked pages")

    print("[2] embed variants + control")
    variants = {}
    for vi, seed in enumerate(args.seeds, start=1):
        pdf_bytes, meta = vector_mark.embed(src, seed, pages=pages)
        variants[f"v{vi}"] = (pdf_bytes, meta)
        open(os.path.join(args.out, f"v{vi}.pdf"), "wb").write(pdf_bytes)
        json.dump(meta, open(os.path.join(args.out, f"v{vi}_meta.json"),
                             "w"), indent=1)
        applied = sum(1 for pg in meta["pages"] for s in pg["slots"]
                      if s["delta_px"] != 0)
        print(f"  v{vi}: seed={seed} slots={meta['n_slots']} "
              f"applied={applied}")
    ctrl_bytes, ctrl_meta = vector_mark.embed(src, args.seeds[0],
                                             pages=pages, unmarked=True)
    if ctrl_bytes != src:
        fail("control is not a byte copy of the source")
    open(os.path.join(args.out, "control.pdf"), "wb").write(ctrl_bytes)
    json.dump(ctrl_meta, open(os.path.join(args.out, "control_meta.json"),
                              "w"), indent=1)
    print("  control: byte copy of source  OK")

    print("[3] formatting guarantee (300-dpi render diff)")
    for vname, (pdf_bytes, meta) in variants.items():
        for pg in meta["pages"]:
            pno = pg["page_index"]
            a0 = render_gray(src, pno).astype(np.int16)
            a1 = render_gray(pdf_bytes, pno).astype(np.int16)
            if a0.shape != a1.shape:
                fail(f"{vname} p{pno}: render dimensions changed")
            diff = np.abs(a0 - a1) > 8          # ignore AA jitter
            rows = np.where(diff.any(axis=1))[0]
            # a marked line owns up to half of each neighboring gap (the
            # embedder's containment rule), plus the shift itself
            runs = pg["runs"]
            spans = []
            for s in pg["slots"]:
                if s["delta_px"] == 0:
                    continue
                j = s["runs_idx"][1]
                a, b = runs[j]
                gu = a - runs[j - 1][1] if j > 0 else vector_mark.MIN_GAP
                gd = (runs[j + 1][0] - b if j + 1 < len(runs)
                      else vector_mark.MIN_GAP)
                ext = vector_mark.SHIFT_PX + 3
                spans.append((a - min(25, gu // 2) - ext,
                              b + min(25, gd // 2) + ext))
            stray = [int(r) for r in rows
                     if not any(lo <= r < hi for lo, hi in spans)]
            if stray:
                fail(f"{vname} p{pno}: {len(stray)} changed rows outside "
                     f"marked bands, e.g. {stray[:6]}")
            print(f"  {vname} p{pno}: {int(diff.sum())} px changed, all "
                  f"inside {len(spans)} marked line bands")
        dsrc = fitz.open(stream=src, filetype="pdf")
        dmk = fitz.open(stream=pdf_bytes, filetype="pdf")
        for pno in range(args.pages, min(args.pages + 2, dsrc.page_count)):
            if dsrc[pno].get_pixmap(dpi=72).samples != \
                    dmk[pno].get_pixmap(dpi=72).samples:
                fail(f"{vname} p{pno}: unmarked page render changed")
        dsrc.close()
        dmk.close()
        print(f"  {vname}: unmarked pages render byte-identical  OK")

    print("[4] decode matrix (synthetic channel; logic verification only)")
    presets = ["clean", "whatsapp", "double"]
    results = []
    for vname, (pdf_bytes, meta) in variants.items():
        other = "v2" if vname == "v1" else "v1"
        ometa = variants[other][1]
        for pi, pg in enumerate(meta["pages"]):
            pno = pg["page_index"]
            if not any(s["delta_px"] != 0 for s in pg["slots"]):
                continue
            png = os.path.join(wd, f"{vname}_p{pno}.png")
            render_png(pdf_bytes, pno, png)
            for preset in presets:
                pwd = os.path.join(wd, f"{vname}_p{pno}_{preset}")
                os.makedirs(pwd, exist_ok=True)
                cap, _ = channel_sim.simulate(png, pwd, preset)
                obs = vector_mark.observe(cap, pg)
                if not obs.get("ok"):
                    results.append((vname, pno, preset, None, None, None,
                                    obs.get("reason")))
                    continue
                ok, tot = vector_mark.score(obs, pg)
                xok, xtot = vector_mark.score(obs, ometa["pages"][pi])
                results.append((vname, pno, preset, (ok, tot),
                                (xok, xtot), obs["quality"], None))

    print(f"\n  {'variant':8s}{'page':6s}{'preset':10s}"
          f"{'true-acc':12s}{'cross-acc':12s}{'quality':8s}")
    agg = {}
    for vname, pno, preset, true, cross, q, err in results:
        if err:
            print(f"  {vname:8s}{pno:<6d}{preset:10s}OBS-FAIL: {err}")
            continue
        ta = f"{true[0]}/{true[1]}={true[0]/true[1]:.2f}" if true[1] else "-"
        xa = (f"{cross[0]}/{cross[1]}={cross[0]/cross[1]:.2f}"
              if cross[1] else "-")
        print(f"  {vname:8s}{pno:<6d}{preset:10s}{ta:12s}{xa:12s}{q:<8.2f}")
        a = agg.setdefault(preset, [0, 0, 0, 0])
        a[0] += true[0]
        a[1] += true[1]
        a[2] += cross[0]
        a[3] += cross[1]

    print("\n  aggregate per preset:")
    verdicts = {}
    for preset in presets:
        a = agg.get(preset, [0, 0, 0, 0])
        t = a[0] / a[1] if a[1] else 0.0
        x = a[2] / a[3] if a[3] else 0.0
        verdicts[preset] = {"true_ok": a[0], "true_tot": a[1],
                            "true_acc": round(t, 3), "cross_ok": a[2],
                            "cross_tot": a[3], "cross_acc": round(x, 3)}
        print(f"  {preset:10s} true {a[0]}/{a[1]} = {t:.3f}   "
              f"cross {a[2]}/{a[3]} = {x:.3f}")

    print("[5] control (unmarked) decode — must sit in the chance band")
    ctl_agree = ctl_tot = 0
    for pg in variants["v1"][1]["pages"]:
        if not any(s["delta_px"] != 0 for s in pg["slots"]):
            continue
        pno = pg["page_index"]
        png = os.path.join(wd, f"ctrl_p{pno}.png")
        render_png(ctrl_bytes, pno, png)
        pwd = os.path.join(wd, f"ctrl_p{pno}_whatsapp")
        os.makedirs(pwd, exist_ok=True)
        cap, _ = channel_sim.simulate(png, pwd, "whatsapp")
        obs = vector_mark.observe(cap, pg)
        if not obs.get("ok"):
            continue
        for si, sl in enumerate(pg["slots"]):
            ob = obs["bits"].get(si)
            if ob is None:
                continue
            ctl_agree += int(ob == sl["bit"])
            ctl_tot += 1
    ctl_acc = ctl_agree / ctl_tot if ctl_tot else None
    print(f"  control vs v1 meta (whatsapp): {ctl_agree}/{ctl_tot}"
          f"{'' if ctl_acc is None else f' = {ctl_acc:.3f}'}"
          f"  ({'chance band' if ctl_acc is None or 0.25 <= ctl_acc <= 0.75 else 'OUT OF BAND — decoder reads layout, results invalid'})")
    verdicts["control_whatsapp"] = {"agree": ctl_agree, "tot": ctl_tot,
                                    "acc": ctl_acc}

    wa = verdicts.get("whatsapp", {})
    t = wa.get("true_acc", 0.0)
    x = wa.get("cross_acc", 0.0)
    print(f"\n  WhatsApp-preset verdict: true-variant {t:.3f} "
          f"({'>= 0.90 target met' if t >= 0.90 else 'BELOW 0.90 target'}), "
          f"cross-variant {x:.3f} "
          f"({'chance band' if 0.25 <= x <= 0.75 else 'OUT OF CHANCE BAND'})")
    print("  (synthetic channel: decoder-logic verification, not a "
          "physical result)")

    json.dump({"results": [
        {"variant": v, "page": p, "preset": pr, "true": t2, "cross": c,
         "quality": q, "error": e}
        for v, p, pr, t2, c, q, e in results],
        "aggregate": verdicts},
        open(os.path.join(args.out, "decode_results.json"), "w"), indent=1)


if __name__ == "__main__":
    main()
